...
describe('Registry edge cases', () => {
  describe('PublicAgentRegistry', () => {
    ...
    it('should handle search errors', async () => {
      ...
      await expect(registry.searchAgents('test'))
        .rejects.toThrow('Failed to search agents: 500 Internal Server Error');
    });
    ...
    it('should validate agent card from registry', async () => {
      ...
      await expect(registry.getAgentCard('invalid-agent'))
        .rejects.toThrow('Invalid agent card from registry');
    });
  });
  ...
  describe('EnterpriseAgentRegistry', () => {
    ...
    it('should handle get agent card errors', async () => {
      ...
      await expect(registry.getAgentCard('agent-123'))
        .rejects.toThrow('Failed to get agent card: 403 Forbidden');
    });
  });
});