...
function MessageComponent({ message, agentName = 'Agent', userName = 'You' }: MessageProps) {
  ...
  const renderContent = () => {
    if (isUser) {
      return <div className="textContent">{message.content}</div>;
    }
    ...
    return (
      <div
        className={`markdownContent ${isArtifact ? 'artifactContent' : ''}`}
  ...
  return (
    <div
      className={`messageWrapper ${isUser ? 'user' : 'assistant'} ${isArtifact ? 'artifact' : ''} chat-fade-in`}
    >
        ...
        <div className="messageBubble">
          <div className="message">
            {isGroupedArtifact && message.metadata?.artifacts ? (
              <div className="groupedArtifactContainer">
                  ...
                  {message.metadata.artifacts.map(
                    (
                      ...
                      <div key={index} className="artifactItem">
                        <div className="artifactItemInfo">
                          <svg
                            className="fileIcon"
                            width="20"
                            height="20"
                          ...
                          <span className="artifactItemName">{artifact.name}</span>
                        </div>
                        <div className="artifactItemActions">
                          <button
              ...
              <div className="artifactContainer">
                <div className="artifactHeader">
                  <div className="artifactInfo">
                    ...
                    <span className="artifactName">{artifactName}</span>
                  </div>
                ...
                {showContent && <div className="artifactContentWrapper">{renderContent()}</div>}
              </div>
            ) : (
              renderContent()
            )}
              ...
              <div className="attachments">
                {message.attachments.map((attachment) => (
                  <div key={attachment.id} className="attachment">
                    ...
                    <span className="attachmentSize">({formatFileSize(attachment.size)})</span>
                  </div>
                ))}
              </div>
            )}
          </div>
          {!isUser && <div className="messageTail" />}
        </div>
        <div className="metadata">
          <span className="time">{formatTime(message.timestamp)}</span>
          {message.status === 'error' && <span className="error">Failed to send</span>}
        </div>
      </div>
    </div>
  );
