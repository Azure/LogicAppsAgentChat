...
import type { AuthConfig, HttpClientOptions } from './types';
  ...
  message = {
    send: async (request: MessageSendRequest): Promise<Task> => {
      ...
      if (jsonRpcResponse.error) {
        throw new Error(`JSON-RPC error: ${jsonRpcResponse.error.message || JSON.stringify(jsonRpcResponse.error)}`);
      }
                ...
                if (!sseClient) {
                  ...
                  const messageHandler = (message: SSEMessage) => {
                    try {
                      ...
                      console.log('DEBUG: Raw SSE message data:', JSON.stringify(jsonRpcData, null, 2));
                      
                      // Check if it's a JSON-RPC error
                      if (jsonRpcData.error) {
                        errorOccurred = new Error(`JSON-RPC error: ${jsonRpcData.error.message || JSON.stringify(jsonRpcData.error)}`);
                        isComplete = true;
                        if (sseClient) {
                          sseClient.close();
                        }
                      ...
                      if (!result.kind && result.id && result.state) {
                        ...
                        // Update task state
                        currentTask.state = result.status?.state === 'completed' ? 'completed' : 
                                          result.status?.state === 'failed' ? 'failed' : 'running';
                        ...
                        if (result.artifact && result.artifact.parts) {
                          ...
                          const existingArtifactIndex = currentTask.artifacts?.findIndex(a => a.id === artifact.id);
